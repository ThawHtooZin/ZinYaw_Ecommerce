...
### 2.3 Admin Portal (Master Control Center & Admin-Vendor Dual Role)
* **Dual Admin-Vendor Capability:** The Admin user possesses full Vendor privileges implicitly. Admin accounts can sell directly on the platform, create store listings, manage inventory, and list products in the global catalog without requiring separate vendor accounts or approval queues (auto-approved).
...
* **Step 1:** Vendor submits **Base Product Request** (Title, Category, Brand, Specification Template, Reference Images). (Note: Admin-submitted base products bypass moderation and auto-approve).
* **Step 2:** Admin approves request -> Product enters the **Global Master Catalog**.
* **Step 3:** Any authorized vendor (or Admin) can attach a **Vendor Listing** to this Base Product by setting:
  * Retail Price (Buyer Coins)
  * Inventory Stock (Requires Vendor Token consumption for standard vendors; bypassable or auto-credited for Admin)
...
* **[LOG-006] [ARCHITECT-STAGE-1]** Updated SRS to v2 (`stage-1-srs-v2.md`) incorporating Admin-Vendor Dual Capability & auto-approval privileges.
