...
### Layout Shells:
1. **`MainLayout`**: Customer storefront frame with top Navbar (MMK Wallet balance badge, live cart drawer trigger, search) and Footer.
2. **`AuthLayout`**: Centered card layout for authentication flows (Login, Register).
3. **`VendorLayout`**: Dedicated merchant portal with sidebar navigation, store header, Vendor Tokens quota badge, and withdrawable MMK balance badge.
4. **`AdminLayout`**: Dark-themed control center sidebar for system moderation queues (Base Products, Bank Slips, Cashout payouts).
...
* **UI Blueprint**:
  * Centered Card with ZinYaw logo.
  * Phone Number / Email input.
  * Password input with visibility toggle.
  * "Login" Button (Electric Indigo `indigo-600`).
...
* **Access Scope**: Public
* **UI Blueprint**:
  * Role Selector Tabs: "Customer Buyer" vs. "Vendor Merchant".
  * Full Name, Phone Number, Email, Password, Password Confirmation.
  * Vendor-specific fields (if Vendor selected): Store Name, Business Registration Number, Bank Account Number (KBZPay/AYA Pay).
...
* **UI Blueprint**:
  * Top Metric Grid (4 Cards):
    1. Total Sales Revenue MMK.
    2. **Withdrawable Balance MMK** (Escrow funds released from completed orders).
    3. **Pending Escrow Holdings MMK** (Orders shipped or awaiting customer delivery confirmation).
    4. **Active Vendor Tokens Quota** (e.g., `12 Tokens Remaining`).
...
* **UI Blueprint**:
  * System Counter Cards:
    1. Pending Base Product Requests Queue Count.
    2. Pending Bank Slips Verification Count.
    3. Pending Vendor Cashout Requests Count.
    4. Total Platform Gross Merchandise Value (GMV) MMK.
...
```text
Customer registers -> Visits /account/wallet -> Opens TopupModal -> Transfers KBZPay MMK -> Uploads Slip 
  -> Admin verifies on /admin/payments/bank-slips -> Customer MMK Balance credited 
  -> Visits Product Page -> Selects Variant & Vendor -> Adds to Cart 
  -> Visits /checkout -> Clicks "Lock Escrow & Place Order" 
  -> Order created with status 'paid' (Funds locked in Escrow)
```

### Flow B: Vendor Fulfillment & Escrow Release Journey
```text
Vendor visits /vendor/orders -> Sees new order item -> Clicks "Ship Item" 
  -> Inputs Courier Name & Tracking # in ShipmentModal -> Order status updates to 'shipped' 
  -> Customer receives package -> Visits /account/orders -> Clicks "Confirm Package Received" 
  -> Status updates to 'completed' -> System releases MMK Escrow funds to Vendor's withdrawable wallet 
  -> Vendor visits /vendor/wallet -> Requests Cash-Out to KBZPay -> Admin marks paid
```
