...
## Stage 3: Laravel REST API & React Integration Blueprint (v2.0)
...
```json
{
  ...
  "data": { "listing_id": 501, "price_coins": 14000.00, "stock_quantity": 70 }
}
```
...
* **HTTP Method:** `POST`
* **Endpoint:** `/orders/items/{item_id}/confirm-delivery`
...
* **`POST /admin/payments/slips/{id}/verify`** — Approve bank slip and credit user's Buyer Coins.

### 7.5 Review & Complete Vendor Cash-Outs
* **`GET /admin/cashouts/pending`** — List pending vendor withdrawal requests.
* **`POST /admin/cashouts/{id}/complete`** — Mark payout complete after transferring funds via KBZPay/AYA Pay.
